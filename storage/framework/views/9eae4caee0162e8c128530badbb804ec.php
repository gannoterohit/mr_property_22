<?php
    ...
    $navGroups = [
        ...
        'content' => ['label' => 'Content Management', 'icon' => 'fa-pen-to-square', 'items' => [
            ...
            ['route' => 'admin.pages.faq', 'match' => 'admin.pages.faq*', 'icon' => 'fa-circle-question', 'label' => 'FAQ'],
...
<aside id="adminSidebar" class="fixed lg:sticky top-0 left-0 z-[60] h-screen w-[280px] bg-white text-slate-700 border-r border-slate-200 flex flex-col -translate-x-full lg:translate-x-0 transition-transform duration-200 shrink-0 shadow-[6px_0_24px_rgba(15,23,42,0.06)]">
        ...
        <a href="<?php echo e(route('admin.dashboard')); ?>" class="flex items-center gap-2.5">
            <?php if($adminLogo): ?>
                <span class="w-9 h-9 rounded-lg bg-white border border-slate-200 flex items-center justify-center overflow-hidden shadow-sm p-1">
                    <img src="<?php echo e(asset('storage/' . $adminLogo)); ?>" alt="<?php echo e($adminSiteName); ?>" class="w-full h-full object-contain">
                </span>
            <?php else: ?>
            ...
            <?php endif; ?>
            <span class="min-w-0"><strong class="block text-slate-900 text-[15px] leading-tight truncate max-w-[175px]"><?php echo e($adminSiteName); ?></strong><small class="block text-[9px] text-slate-500 tracking-[.14em] uppercase mt-1 font-bold">Admin Workspace</small></span>
        </a>
    ...
    <nav class="flex-1 min-h-0 overflow-y-auto px-3 py-3 space-y-2 overscroll-contain">
        <?php if(!Auth::user()?->admin_role_id || Auth::user()?->hasAdminPermission('dashboard.view')): ?>
            <a href="<?php echo e(route('admin.dashboard')); ?>" class="admin-sidebar-idle-icon-hover group relative flex items-center gap-3 rounded-xl px-3 py-3 text-[13px] transition <?php echo e(request()->routeIs('admin.dashboard') ? 'admin-sidebar-active font-bold' : 'text-slate-700 font-semibold hover:bg-slate-100 hover:text-slate-950'); ?>">
                <span class="flex h-8 w-8 items-center justify-center rounded-lg <?php echo e(request()->routeIs('admin.dashboard') ? 'bg-white/15 text-white' : 'bg-slate-100 text-slate-500'); ?>"><i class="fas fa-th-large text-[12px]"></i></span>
                <span>Dashboard</span>
            </a>
        <?php endif; ?>
        ...
        <?php $__currentLoopData = $navGroups; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $groupKey => $group): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php
                $adminUser = Auth::user();
            ...
            ?>
            <?php if($groupItems->isEmpty()) continue; ?>
            <?php
            ...
            ?>
            <section class="admin-nav-group" data-group="<?php echo e($groupKey); ?>">
                <button type="button" class="admin-nav-group-toggle group flex w-full items-center justify-between rounded-xl border px-3 py-2.5 text-[13px] font-bold transition <?php echo e($groupActive ? 'admin-sidebar-group-active' : 'border-transparent bg-white text-slate-700 hover:bg-slate-50'); ?>" aria-expanded="<?php echo e($groupOpen ? 'true' : 'false'); ?>">
                    <span class="flex min-w-0 items-center gap-2.5">
                        <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg shadow-sm <?php echo e($groupActive ? 'admin-sidebar-icon-active' : 'admin-sidebar-icon-idle ring-1 ring-slate-200'); ?>"><i class="fas <?php echo e($group['icon']); ?> text-[12px]"></i></span>
                        <span class="whitespace-nowrap text-[12px]"><?php echo e($group['label']); ?></span>
                    </span>
                    <span class="flex items-center gap-1.5">
                        <?php if($groupBadgeCount > 0): ?>
                            <span class="sidebar-unread-badge rounded-full bg-red-500 px-1.5 py-0.5 text-[10px] font-extrabold text-white">
                                <?php echo e($groupBadgeCount > 99 ? '99+' : $groupBadgeCount); ?>

                            </span>
                        <?php endif; ?>
                        <i class="admin-nav-chevron fas fa-chevron-down text-[9px] text-slate-400 transition-transform <?php echo e($groupOpen ? 'rotate-180' : ''); ?>"></i>
                    </span>
                </button>
                <div class="admin-sidebar-submenu admin-nav-group-menu <?php echo e($groupOpen ? '' : 'hidden'); ?> ml-6 mt-2 space-y-1.5 border-l-2 pl-3">
                    <?php $__currentLoopData = $groupItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        ?>
                        <a href="<?php echo e(route($item['route'])); ?>" class="relative flex items-center justify-between gap-2.5 rounded-lg px-3 py-2.5 text-[12px] transition <?php echo e($itemActive ? 'admin-sidebar-subitem-active font-extrabold' : 'text-slate-600 font-semibold hover:bg-slate-50 hover:text-slate-900'); ?>">
                            <span class="flex items-center gap-2.5 min-w-0">
                                <i class="fas <?php echo e($item['icon']); ?> w-4 text-center text-[11px] <?php echo e($itemActive ? 'admin-sidebar-active-icon' : 'text-slate-400'); ?>"></i>
                            ...
                            <?php if($itemBadge > 0): ?>
                                <span class="sidebar-unread-badge shrink-0 rounded-full bg-red-500 px-1.5 py-0.5 text-[10px] font-extrabold text-white">
                                    <?php echo e($itemBadge > 99 ? '99+' : $itemBadge); ?>
    ...
    <div id="adminSidebarFooter" class="shrink-0 p-2.5 border-t border-slate-200 bg-white shadow-[0_-6px_18px_rgba(15,23,42,0.04)]">
        <div class="flex items-center gap-2.5 px-2 py-1.5 mb-1.5 rounded-lg bg-slate-50 border border-slate-100">
            <?php if(Auth::user()->avatar): ?>
                <img src="<?php echo e(asset('storage/' . Auth::user()->avatar)); ?>" alt="<?php echo e(Auth::user()->name); ?>" width="200" height="200" class="w-8 h-8 rounded-lg object-cover ring-2 ring-white shadow-sm">
            <?php else: ?>
                <div class="admin-theme-avatar w-8 h-8 shrink-0 rounded-lg flex items-center justify-center font-bold text-xs ring-2 ring-white shadow-sm"><?php echo e(strtoupper(substr(Auth::user()->name, 0, 1))); ?></div>
            <?php endif; ?>
            <div class="min-w-0 flex-1"><p class="text-xs font-bold text-slate-800 truncate"><?php echo e(Auth::user()->name); ?></p><p class="text-[8px] text-slate-400 font-semibold uppercase tracking-wider">Administrator</p></div>
        </div>
...
<?php /**PATH C:\xampp\htdocs\apnanest\resources\views/admin/partials/sidebar.blade.php ENDPATH**/ ?>