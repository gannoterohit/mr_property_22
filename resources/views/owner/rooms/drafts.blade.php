...
<div class="max-w-5xl mx-auto px-4 py-6">
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('owner.rooms.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-700 transition shadow-md">
    ...
    @if($drafts->isEmpty())
        <div class="bg-white rounded-2xl border border-slate-200 p-12 text-center">
            ...
            <a href="{{ route('owner.rooms.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-700 transition">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach($drafts as $draft)
                <div class="bg-white rounded-2xl border border-slate-200 p-5 hover:shadow-md transition group">
                    ...
                    <a href="{{ route('owner.rooms.create') }}?draft={{ $draft->id }}" class="block w-full text-center px-3 py-2 rounded-lg bg-indigo-50 text-indigo-700 text-xs font-bold hover:bg-indigo-100 transition">
...
document.querySelectorAll('.delete-draft').forEach(btn => {
    btn.addEventListener('click', async () => {
        ...
        try {
            const res = await fetch(`/owner/rooms/drafts/${id}`, {
                method: 'DELETE',
                headers: {
