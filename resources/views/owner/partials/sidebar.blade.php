@php
    ...
    $navGroups = [
        'main' => ['label' => 'Main', 'icon' => 'fa-th-large', 'items' => [
            ['route' => 'owner.dashboard', 'match' => 'owner.dashboard', 'icon' => 'fa-chart-pie', 'label' => 'Dashboard'],
        ]],
    ...
    $financialGroup = $navGroups['financial']['items'];
    if (!\App\Models\Setting::isEnabled('wallet_enabled', true)) {
        $navGroups['financial'] = ['label' => 'Financial', 'icon' => 'fa-wallet', 'items' => []];
    }
...
<aside class="owner-sidebar hidden lg:flex bg-white border-r border-slate-200 flex-col sticky top-0 h-screen">
    <div class="px-5 py-3 border-b border-slate-100">
        <a href="{{ route('owner.dashboard') }}" class="flex items-center gap-3">
            @if($ownerLogo)
                <img src="{{ asset('storage/' . $ownerLogo) }}" alt="Owner panel" class="w-8 h-8 rounded-xl object-contain border border-slate-200 bg-white p-1">
            @else
            ...
            @endif
            <span class="min-w-0">
                <strong class="block text-[13px] text-slate-900 truncate">Owner Workspace</strong>
                <small class="block text-[10px] font-bold uppercase tracking-wider text-slate-400">Manage properties</small>
            </span>
        </a>
    </div>

    <nav class="flex-1 min-h-0 overflow-y-auto pt-4 px-3 pb-3 space-y-1 overscroll-contain" aria-label="Owner navigation">
    ...
    <div class="p-3 border-t border-slate-100">
        <div class="flex items-center gap-3 px-3 py-2 mb-1">
            @if($owner?->avatar)<img src="{{ asset('storage/'.$owner->avatar) }}" width="200" height="200" alt="Owner profile" class="w-8 h-8 rounded-full border border-slate-200 owner-theme-soft object-cover">@else<div class="w-8 h-8 rounded-full border border-slate-200 owner-theme-soft flex items-center justify-center"><i class="fas fa-user-tie" aria-hidden="true"></i><span class="sr-only">Owner profile</span></div>@endif
            <span class="min-w-0">
