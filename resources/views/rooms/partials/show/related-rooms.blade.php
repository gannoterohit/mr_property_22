        ...
        <div class="related-room-grid">
            ...
            <a href="{{ route('rooms.show', $relatedRoom) }}" class="related-room-card">
                <div class="related-room-image">
                     <img src="{{ $relatedRoom->photo_url }}" alt="{{ $relatedRoom->title }} in {{ $relatedRoom->city }}" width="400" height="300" loading="lazy" onerror="this.src='{{ asset('assets/images/default-room.svg') }}'">
                ...
                <div class="related-room-copy">
                    <div>
                        <small>{{ $relatedRoom->roomTypeLabel() }}</small>
                        <strong>&#8377;{{ number_format((float)$relatedRoom->rent) }}<em>/mo</em></strong>
                    </div>
                    <h3>{{ $relatedRoom->title }}</h3>
                    <p><i class="fas fa-location-dot"></i>{{ $relatedRoom->city }}</p>
                    <div class="related-room-meta">
                        <span><i class="fas fa-couch"></i>{{ $relatedRoom->furnishingTypeLabel() }}</span>
                        <span><i class="fas fa-user"></i>{{ $relatedRoom->tenantTypeLabel() }}</span>
                        @if($relatedRoom->propertyType?->name)
                            <span><i class="fas fa-building"></i>{{ $relatedRoom->propertyType->name }}
                                @if($relatedRoom->propertyCategory?->name)
                                    · {{ $relatedRoom->propertyCategory->name }}
                                @endif
                            </span>
                        @endif
                        @if($relatedRoom->area_sqft)
                            <span><i class="fas fa-ruler-combined"></i>{{ number_format((float)$relatedRoom->area_sqft, 2) }} sqft</span>
                        @endif
                    </div>
                </div>
            </a>
