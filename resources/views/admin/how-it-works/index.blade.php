                    ...
                    <tbody class="divide-y">
                        @forelse(($items[$groupKey] ?? collect()) as $item)
                            <tr>
                                ...
                                <td class="p-4"><span class="rounded-lg px-2.5 py-1 text-[10px] font-extrabold {{ $item->is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500' }}">{{ $item->is_active ? 'Active' : 'Inactive' }}</span></td>
                                <td class="p-4"><div class="flex justify-end gap-2"><a href="{{ route('admin.how-it-works.items.edit', $item) }}" class="rounded-lg admin-theme-soft px-3 py-2 text-xs font-bold admin-theme-text"><i class="fas fa-edit mr-1"></i>Edit</a><form method="POST" action="{{ route('admin.how-it-works.items.toggle-status', $item) }}">@csrf @method('PATCH')<button class="rounded-lg bg-slate-50 px-3 py-2 text-xs font-bold text-slate-700">{{ $item->is_active ? 'Disable' : 'Enable' }}</button></form><form method="POST" action="{{ route('admin.how-it-works.items.destroy', $item) }}" class="admin-confirm" data-confirm-title="Delete {{ $item->title }}?" data-confirm-text="This item will be permanently removed." data-confirm-button="Yes, delete item">@csrf @method('DELETE')<button class="rounded-lg bg-red-50 px-3 py-2 text-xs font-bold text-red-700"><i class="fas fa-trash"></i></button></form></div></td>
                            </tr>
                        @empty
