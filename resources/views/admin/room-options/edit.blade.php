...
<div class="max-w-2xl mx-auto space-y-5">
    ...
    <form action="{{ route('admin.room-options.update', $roomOption) }}" method="POST" class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="p-5 space-y-5">
            ...
            <div><label class="block text-xs font-bold text-slate-600 mb-1.5">Status</label><select name="is_active" required class="w-full rounded-lg text-sm bg-white"><option value="1" @selected((string) old('is_active', (int) $roomOption->is_active) === '1')>Active</option><option value="0" @selected((string) old('is_active', (int) $roomOption->is_active) === '0')>Inactive</option></select><p class="text-[11px] text-slate-400 mt-1">Inactive options stay saved for old listings but are hidden from new forms and API.</p></div>
        </div>
        <div class="px-5 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3"><a href="{{ route('admin.room-options.index') }}" class="px-4 py-2.5 rounded-lg border border-slate-200 bg-white text-slate-600 text-xs font-bold">Cancel</a><button type="submit" class="px-5 py-2.5 rounded-lg admin-theme-bg text-xs font-bold"><i class="fas fa-save mr-1.5"></i>Update option</button></div>
    </form>
</div>
@endsection
