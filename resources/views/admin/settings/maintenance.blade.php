...
@php
    ...
    $controls = [
        ...
        ['broker_module_enabled', 'Broker module', 'Allow brokers and agents to register, login and manage listings.', 'fa-handshake', '1'],
    ];
@endphp
    ...
    <form action="{{ route('admin.maintenance.update') }}" method="POST" class="space-y-6">@csrf
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div class="mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <div>
                    <h2 class="font-extrabold text-slate-900">Module availability</h2>
                    <p class="mt-1 text-sm text-slate-500">Keep unaffected parts of the business running.</p>
                </div>
                <a href="{{ route('admin.settings') }}#broker" class="text-xs font-bold text-emerald-600 hover:text-emerald-700 inline-flex items-center gap-1.5 bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-200">
                    <i class="fas fa-handshake"></i> Configure Broker Rules & Fees <i class="fas fa-arrow-right text-[10px]"></i>
                </a>
            </div>
            <div class="grid gap-3 lg:grid-cols-2">
                @foreach($controls as [$key,$label,$description,$icon,$default])
                    <div class="flex items-center justify-between gap-4 rounded-xl border border-slate-200 bg-slate-50 p-4 transition">
                        <span class="flex min-w-0 gap-3">
                            <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white admin-theme-text shadow-sm"><i class="fas {{ $icon }}"></i></span>
                            <span>
                        ...
                        <span class="flex shrink-0 items-center gap-2">
                            ...
                            <button type="button" role="switch" :aria-checked="{{ $key }}" @click="{{ $key }} = !{{ $key }}" class="toggle-btn" :class="{{ $key }} ? 'toggle-btn-active' : 'toggle-btn-inactive'">
                                <span class="toggle-track" :class="{{ $key }} ? 'toggle-track-active' : 'toggle-track-inactive'"><span class="toggle-knob" :class="{{ $key }} ? 'toggle-knob-active' : 'toggle-knob-inactive'"></span></span>
                                <span class="toggle-label" x-text="{{ $key }} ? 'ON' : 'OFF'"></span>
                            </button>
                        </span>
                    </div>
                @endforeach
