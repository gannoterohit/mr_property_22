<nav class="flex gap-2 overflow-x-auto rounded-2xl border bg-white p-2 shadow-sm">
    @foreach([
        ...
        <a href="{{ route($routeName) }}" class="inline-flex min-w-max items-center gap-2 rounded-xl px-4 py-2.5 text-xs font-extrabold transition {{ request()->routeIs($match) ? 'bg-indigo-600 text-white shadow-sm shadow-indigo-100' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
