                                ...
                                <div class="flex items-center justify-between gap-4 px-5 py-4">
                                    <div class="min-w-0">
                                        @switch($panelKey)
                                            ...
                                            @case('enquiries') <p class="truncate text-sm font-bold">{{ $record->room?->title ?? 'Deleted property' }}</p><p class="text-xs text-slate-400">{{ $record->unlocked ? 'Contact unlocked' : 'Pending unlock' }}</p> @break
                                            ...
                                            @case('wishlists') <p class="truncate text-sm font-bold">{{ $record->room?->title ?? 'Deleted property' }}</p><p class="text-xs text-slate-400">{{ $record->room?->city ?? 'Location unavailable' }}</p> @break
