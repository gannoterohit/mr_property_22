...
<div class="space-y-5 p-5 lg:p-6">
    ...
    <form class="owner-filter rounded-2xl border bg-white p-4 shadow-sm">
        ...
        <select name="verification_status" class="h-11 rounded-xl border-slate-200 text-sm">
            ...
            @foreach(['pending','under_review','verified','rejected'] as $v)
                <option value="{{ $v }}" @selected(request('verification_status')===$v)>{{ ucfirst(str_replace('_',' ',$v)) }}</option>
            @endforeach
        </select>
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('admin.owners') }}" class="inline-flex h-11 items-center rounded-xl border px-4 text-xs font-extrabold text-slate-600">Reset</a>
        </div>
    </form>
    ...
    <section class="overflow-hidden rounded-2xl border bg-white shadow-sm">
        <div class="flex items-center justify-between border-b px-5 py-4">
            <div>
                <h2 class="text-sm font-extrabold">Owner Directory</h2>
                <p class="text-xs text-slate-500">{{ $owners->total() }} accounts match the current filters</p>
            </div>
            ...
            <table class="people-table admin-table-base">
                ...
                <tbody class="divide-y">
                    @forelse($owners as $owner)
                        @php
                            $tone=match($owner->verification_status){
                        ...
                        <tr class="hover:bg-slate-50/70">
                            <td class="px-5 py-4 whitespace-nowrap">
                                <div class="flex min-w-[220px] items-center gap-3">
                                    <span class="admin-theme-soft flex h-10 w-10 shrink-0 items-center justify-center rounded-xl font-extrabold">{{ strtoupper(substr($owner->name,0,1)) }}</span>
                                    <div class="min-w-0">
                            ...
                            <td class="px-4 py-4"><p class="admin-theme-text text-lg font-extrabold">{{ $owner->rooms_count }}</p><p class="text-[10px] text-slate-400">properties</p></td>
                            ...
                            <td class="px-4 py-4">
                                <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-extrabold {{ $owner->trashed()||$owner->is_blocked?'bg-red-50 text-red-700':'bg-emerald-50 text-emerald-700' }}">
                                    <i class="fas fa-circle text-[5px]"></i>{{ $owner->trashed()?'Deleted':($owner->is_blocked?'Blocked':'Active') }}
                                </span>
                            </td>
                            ...
                            <td class="px-5 py-4">
                                ...
                                @else
                                    <div class="flex justify-end gap-2">
                                        ...
                                        <form method="POST" action="{{ route('admin.owners.destroy',$owner) }}" class="admin-confirm" data-confirm-title="Delete {{ $owner->name }}?" data-confirm-text="The account will be soft deleted and can be restored from the Deleted filter. Listings are retained." data-confirm-button="Yes, delete owner">
                                            @csrf
                                            @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="7" class="p-14 text-center"><i class="fas fa-users-slash text-3xl text-slate-300"></i><p class="mt-3 text-sm font-bold text-slate-600">No owners found</p><p class="text-xs text-slate-400">Try changing the filters.</p></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
