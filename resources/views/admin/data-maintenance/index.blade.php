...
<div class="space-y-6 p-5 lg:p-7">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div><p class="text-xs font-bold uppercase tracking-widest text-indigo-600">System health</p><h1 class="mt-1 text-2xl font-extrabold text-slate-950">Data Maintenance</h1><p class="mt-1 text-sm text-slate-500">Control data growth without removing genuine users, rooms, completed payments or unlocked contacts.</p></div>
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div class="rounded-2xl border bg-white p-5 shadow-sm"><span class="flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600"><i class="fas fa-database"></i></span><p class="mt-4 text-xs font-bold uppercase text-slate-400">Database size</p><strong class="mt-1 block text-xl text-slate-900">{{ $formatBytes($summary['database_bytes']) }}</strong></div>
        <div class="rounded-2xl border bg-white p-5 shadow-sm"><span class="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-50 text-violet-600"><i class="fas fa-hard-drive"></i></span><p class="mt-4 text-xs font-bold uppercase text-slate-400">Public storage</p><strong class="mt-1 block text-xl text-slate-900">{{ $formatBytes($summary['storage_bytes']) }}</strong></div>
    ...
    <div class="grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
        <form action="{{ route('admin.data-maintenance.update') }}" method="POST" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:p-6">@csrf @method('PUT')
            ...
            <button class="mt-5 rounded-xl bg-indigo-600 px-5 py-3 text-sm font-bold text-white hover:bg-indigo-700"><i class="fas fa-floppy-disk mr-2"></i>Save retention settings</button>
        </form>
        ...
        <div class="space-y-6">
            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:p-6"><div class="flex items-start justify-between gap-4"><div><h2 class="font-extrabold text-slate-900">Orphan room media</h2><p class="mt-1 text-sm text-slate-500">Files under room storage that are not linked to any room.</p></div><span class="rounded-full bg-violet-50 px-3 py-1 text-xs font-bold text-violet-700">{{ count($orphanMedia) }} files · {{ $formatBytes($orphanBytes) }}</span></div>@if(count($orphanMedia))<div class="mt-4 max-h-44 space-y-2 overflow-y-auto">@foreach(array_slice($orphanMedia,0,20) as $file)<div class="flex items-center justify-between gap-3 rounded-lg bg-slate-50 px-3 py-2 text-[11px]"><span class="truncate text-slate-600" title="{{ $file['path'] }}">{{ $file['path'] }}</span><strong class="shrink-0 text-slate-500">{{ $formatBytes($file['bytes']) }}</strong></div>@endforeach</div>@else<div class="mt-4 rounded-xl bg-emerald-50 p-4 text-xs font-semibold text-emerald-700"><i class="fas fa-circle-check mr-2"></i>No orphan room media found.</div>@endif</section>
        </div>
    </div>
    ...
    <form action="{{ route('admin.data-maintenance.cleanup') }}" method="POST" onsubmit="return confirm('Run cleanup using the preview shown above? This cannot be undone.')" class="rounded-2xl border border-amber-200 bg-amber-50 p-5 lg:p-6">@csrf
        <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div><h2 class="font-extrabold text-amber-950"><i class="fas fa-triangle-exclamation mr-2"></i>Run cleanup now</h2><p class="mt-1 text-sm text-amber-800">Only eligible temporary records shown above will be removed. Successful payments, real unlocks, rooms and members remain safe.</p><label class="mt-4 flex items-center gap-2 text-xs font-bold text-amber-950"><input type="checkbox" name="delete_orphan_media" value="1" class="rounded border-amber-300 text-indigo-600">Also delete {{ count($orphanMedia) }} orphan room media files</label></div><div class="shrink-0"><label class="mb-3 flex items-center gap-2 text-xs font-bold text-amber-950"><input type="checkbox" name="confirm_cleanup" value="1" required class="rounded border-amber-300 text-indigo-600">I reviewed the cleanup preview</label><button class="w-full rounded-xl bg-amber-600 px-6 py-3 text-sm font-bold text-white hover:bg-amber-700"><i class="fas fa-broom mr-2"></i>Run Cleanup Now</button></div></div>
    </form>
</div>
@endsection
