            ...
            <table class="offer-table divide-y divide-slate-200">
                ...
                @forelse($offers as $offer)
                    ...
                    <tr class="hover:bg-slate-50/60">
                        ...
                        <td class="px-5 py-4"><div class="flex items-center gap-1.5"><a href="{{ route('admin.offers.edit', $offer) }}" title="Edit offer" class="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 text-slate-600  hover:bg-slate-50 admin-theme-hover-text"><i class="fas fa-pen text-xs"></i></a><form action="{{ route('admin.offers.toggleActive', $offer) }}" method="POST" class="admin-confirm" data-confirm-title="{{ $offer->is_active ? 'Deactivate' : 'Activate' }} {{ $offer->title }}?" data-confirm-text="The offer visibility will be updated immediately." data-confirm-button="Yes, {{ $offer->is_active ? 'deactivate' : 'activate' }}" data-confirm-color="{{ $offer->is_active ? '#dc2626' : '#059669' }}">@csrf<button title="{{ $offer->is_active ? 'Deactivate' : 'Activate' }}" class="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-100"><i class="fas fa-{{ $offer->is_active ? 'pause' : 'play' }} text-xs"></i></button></form><form action="{{ route('admin.offers.destroy', $offer) }}" method="POST" class="admin-confirm" data-confirm-title="Delete {{ $offer->title }}?" data-confirm-text="This offer will be permanently removed." data-confirm-button="Yes, delete offer">@csrf @method('DELETE')<button title="Delete offer" class="flex h-9 w-9 items-center justify-center rounded-lg border border-red-100 text-red-500 hover:bg-red-50"><i class="fas fa-trash text-xs"></i></button></form></div></td>
                    </tr>
                @empty
